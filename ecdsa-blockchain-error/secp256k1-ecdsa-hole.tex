...
\subsection{同一用户重用随机数$k$导致密钥泄露}

\subsection{不同用户重用随机数$k$导致密钥泄露}

\subsection{ECDSA与Schnorr共用$k$导致私钥泄露}

\subsection{ECDSA签名值的可锻造性造成网络分裂}

\subsection{ECDSA签名值DER编码的不唯一造成网络分裂}

\subsection{不需要提供消息$m$时可伪造签名值}
